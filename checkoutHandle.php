...
<?php
   ...
   $user_id=$_SESSION['userid'];
    ...
    while($item_cart_pull=mysqli_fetch_assoc($result_cart_pull)){
        ...
        $sql_item_data="SELECT * from items where item_id=$cart_item_id";
        $result_item_data=mysqli_query($conn,$sql_item_data);
        $row_item_data=mysqli_fetch_assoc($result_item_data);

        $cart_item_image=$row_item_data['item_image'];
        $cart_item_name=$row_item_data["item_name"]; //item name
        $cart_item_price=$row_item_data["item_price"]; //item price
